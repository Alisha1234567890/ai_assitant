...
:root{
  ...
  --radius:13px;
  --sidebar-w: 260px;
  --bg-trans: background 0.3s ease, color 0.3s ease, border-color 0.3s ease, box-shadow 0.3s ease;
}
.dark {
  --white: #12121e; --off-white: #0a0a0f; --light: #1c1c2b; --light2: #252538;
  --text: #f0f0f5; --text-md: #b0b0cc; --text-dim: #70708a;
  --border: rgba(245,184,0,0.12); --border-blue: rgba(26,106,255,0.18); --border-gold: rgba(245,184,0,0.15);
  --grad-soft: linear-gradient(135deg, rgba(245,184,0,0.08) 0%, rgba(240,101,0,0.06) 50%, rgba(26,106,255,0.08) 100%);
}
body{background:var(--off-white);color:var(--text);font-family:var(--sans);font-size:13.5px;-webkit-font-smoothing:antialiased;transition: var(--bg-trans);}
.ic{width:15px;height:15px;flex-shrink:0;}

/* Scrollbar */
::-webkit-scrollbar{width:6px;}
::-webkit-scrollbar-track{background:transparent;}
::-webkit-scrollbar-thumb{background:rgba(240,101,0,0.2);border-radius:99px;}
.dark ::-webkit-scrollbar-thumb{background:rgba(245,184,0,0.15);}
::-webkit-scrollbar-thumb:hover{background:rgba(240,101,0,0.35);}

.shell{display:flex;height:100vh;overflow:hidden;background:var(--off-white);transition: var(--bg-trans);}
...
.sidebar{
  ...
  box-shadow: 3px 0 24px rgba(245,184,0,0.07);
  position: relative;
  z-index: 100;
...
.brand{display:flex;align-items:center;gap:10px;margin-bottom:14px;white-space: nowrap;}
...
.chat-item{border-radius:var(--radius);padding:4px 6px;cursor:pointer;transition:all .15s;margin-bottom:3px;border: 1px solid transparent;}
.chat-item:hover{background:rgba(245,184,0,0.08);transform: translateX(2px);}
.chat-item-active{background:var(--grad-soft)!important;border:1px solid rgba(240,101,0,0.22)!important;}
...
.sidebar-foot{padding:12px 10px 16px;border-top:1.5px solid var(--border-gold);transition: var(--bg-trans);}
.sidebar-user{margin-bottom: 12px; padding: 0 4px; display: flex; flex-direction: column; gap: 2px;}
.sidebar-user strong{font-size: 13px; color: var(--text);}
.sidebar-user span{font-size: 11px; color: var(--text-dim); overflow: hidden; text-overflow: ellipsis; white-space: nowrap;}
.btn-logout{width:100%; margin-bottom: 8px; padding: 7px; border-radius: 8px; border: 1px solid var(--border); background: var(--light); color: var(--text-md); font-size: 11.5px; cursor: pointer; transition: all 0.2s;}
.btn-logout:hover{background: rgba(220,50,50,0.08); color: #d04040; border-color: rgba(220,50,50,0.2);}
...
/* main */
.main{flex:1;display:flex;flex-direction:column;overflow:hidden;background: var(--off-white);transition: var(--bg-trans);}
.topbar{flex-shrink:0;padding:12px 22px;border-bottom:1.5px solid var(--border-gold);background:var(--white);display:flex;align-items:center;justify-content:space-between;box-shadow:0 2px 16px rgba(245,184,0,0.08);gap:12px;transition: var(--bg-trans);z-index: 50;}
...
.btn-theme-toggle {
  ...
  border-radius: 8px; border: 1.5px solid var(--border-gold); background: var(--white);
  color: var(--text-md); cursor: pointer; transition: all 0.2s; flex-shrink: 0;
}
.btn-theme-toggle:hover { border-color: var(--orange); color: var(--orange); background: var(--light); }
...
.tts-panel{position:absolute;top:calc(100% + 8px);right:0;background:var(--white);border:1.5px solid var(--border-gold);border-radius:var(--radius);padding:14px 16px;min-width:220px;z-index:200;box-shadow:0 8px 30px rgba(240,101,0,0.12);display:flex;flex-direction:column;gap:12px;transition: var(--bg-trans);}
...
.tts-select{padding:7px 10px;border-radius:8px;border:1.5px solid var(--border-gold);background:var(--light);font-family:var(--sans);font-size:12.5px;color:var(--text);outline:none;cursor:pointer;transition: var(--bg-trans);}
...
.stt-lang-panel{position:absolute;bottom:calc(100% + 8px);right:0;background:var(--white);border:1.5px solid var(--border-gold);border-radius:var(--radius);padding:12px;z-index:200;box-shadow:0 8px 30px rgba(240,101,0,0.12);min-width:180px;max-height:240px;overflow-y:auto;transition: var(--bg-trans);}
...
/* STT banner */
.stt-banner{display:flex;align-items:center;gap:10px;padding:8px 22px;background:rgba(240,101,0,0.07);border-top:1px solid rgba(240,101,0,0.15);border-bottom:1px solid rgba(240,101,0,0.15);font-size:12.5px;color:var(--orange);font-weight:500;flex-shrink:0;transition: var(--bg-trans);}
...
.stt-interim{display:flex;align-items:center;gap:8px;padding:8px 14px;margin:0 0 10px;border-radius:var(--radius);background:rgba(245,184,0,0.08);border:1px dashed rgba(240,101,0,0.3);color:var(--orange);font-size:13px;font-style:italic;align-self:flex-start;max-width:70%;transition: var(--bg-trans);}
...
/* messages */
.msgs{flex:1;overflow-y:auto;padding:28px 28px;display:flex;flex-direction:column;background:radial-gradient(ellipse 55% 35% at 15% 8%,rgba(245,184,0,0.08) 0%,transparent 65%),radial-gradient(ellipse 45% 30% at 85% 92%,rgba(26,106,255,0.07) 0%,transparent 65%),var(--off-white);transition: var(--bg-trans);}
...
.feat-chip{display:flex;align-items:center;gap:5px;font-size:11.5px;color:var(--text-md);background:var(--white);border:1.5px solid var(--border-gold);padding:5px 12px;border-radius:99px;font-weight:500;transition: var(--bg-trans);}
...
.bubble{max-width:68%;padding:12px 16px;border-radius:16px;font-size:13.5px;line-height:1.7;white-space:pre-wrap;word-break:break-word;transition: var(--bg-trans);}
...
.dark .bubble-user{background:rgba(26,106,255,0.15);color: #60a5ff; border-color: rgba(26,106,255,0.3);}
...
/* input */
.input-row{flex-shrink:0;display:flex;align-items:flex-end;gap:8px;padding:12px 22px 10px;border-top:1.5px solid var(--border-gold);background:var(--white);box-shadow:0 -2px 14px rgba(245,184,0,0.06);transition: var(--bg-trans);}
.input-box{flex:1;resize:none;min-height:44px;max-height:130px;background:var(--light);border:1.5px solid var(--border-gold);border-radius:var(--radius);padding:12px 14px;color:var(--text);font-size:13.5px;font-family:var(--sans);outline:none;transition:all .2s;overflow-y:auto;}
...
/* upload */
.upload-zone{flex-shrink:0;display:flex;flex-direction:column;gap:8px;padding:10px 22px 14px;background:var(--white);border-top:1.5px solid var(--border-blue);transition: var(--bg-trans);}
...
.file-pill{display:flex;align-items:center;gap:6px;padding:5px 11px;border-radius:8px;border:1px solid;font-size:11.5px;font-family:var(--sans);max-width:260px;transition: var(--bg-trans);}
...
.btn-icon-label:hover {
  ...
  color: var(--orange);
  background: rgba(240,101,0,0.05);
}
.btn-icon-label.active {
  border-color: var(--orange)!important;
  background: rgba(240,101,0,0.08)!important;
...
.modal-box{background:var(--white);border:1.5px solid var(--border-gold);border-radius:18px;width:100%;max-width:560px;box-shadow:0 20px 60px rgba(240,101,0,0.15),0 4px 20px rgba(0,0,0,0.1);display:flex;flex-direction:column;gap:0;overflow:hidden;transition: var(--bg-trans);}
...
.modal-textarea{margin:0 20px;border:1.5px solid var(--border-gold);border-radius:var(--radius);padding:12px 14px;font-family:var(--sans);font-size:13px;color:var(--text);background:var(--light);resize:vertical;outline:none;transition:all .2s;min-height:160px;}
...
.chat-pane{flex:1;min-width:0;display:flex;flex-direction:column;overflow:hidden;border-right:1.5px solid var(--border-gold);transition: var(--bg-trans);}
...
.knowledge-pane-modern{width:42%;min-width:320px;max-width:600px;flex-shrink:0;display:flex;flex-direction:column;background:var(--white);overflow:hidden;border-left:1px solid var(--border);box-shadow:-4px 0 24px rgba(0,0,0,0.02);z-index:10;transition: var(--bg-trans);}
.knowledge-head-modern{flex-shrink:0;display:flex;align-items:center;justify-content:space-between;padding:16px 20px;border-bottom:1px solid var(--border);background:var(--white);backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px);transition: var(--bg-trans);}
...
.knowledge-title-modern{font-family:'Inter', sans-serif;font-size:16px;font-weight:700;color:var(--text);letter-spacing:-0.01em;}
.knowledge-subtitle-modern{font-size:12px;color:var(--text-md);margin-top:2px;font-weight:400;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
.knowledge-head-actions{display:flex;align-items:center;gap:10px;}
.knowledge-action-btn{padding:6px 14px;border-radius:8px;border:1px solid var(--border);background:var(--white);font-size:12px;font-weight:600;color:var(--text-md);cursor:pointer;transition:all 0.2s cubic-bezier(0.4, 0, 0.2, 1);}
.knowledge-action-btn:hover:not(:disabled){border-color:#2563eb;color:#2563eb;background:rgba(37,99,235,0.02);}
.knowledge-close-btn{width:32px;height:32px;border-radius:8px;border:1px solid var(--border);background:var(--white);color:var(--text-dim);display:flex;align-items:center;justify-content:center;cursor:pointer;transition:all 0.2s;}
.knowledge-close-btn:hover{background:#fef2f2;border-color:#fecaca;color:#ef4444;}
.knowledge-graph-container{flex:1;min-height:600px;position:relative;background:var(--off-white);width:100%;height:100%;transition: var(--bg-trans);}
.knowledge-footer-modern{flex-shrink:0;display:flex;align-items:center;justify-content:space-between;padding:12px 20px;border-top:1px solid var(--border);background:var(--white);transition: var(--bg-trans);}
...
.stat-label{font-size:11px;color:var(--text-dim);font-weight:500;text-transform:uppercase;letter-spacing:0.02em;}
...
.km-graph-body-only{flex:1;min-height:500px;height:100%;width:100%;position:relative;background:var(--white);transition: var(--bg-trans);}
.km-graph-toolbar{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:12px 20px;border-bottom:1px solid var(--border);background:var(--white);backdrop-filter:blur(8px);transition: var(--bg-trans);}
.km-graph-search-wrap{position:relative;flex:1;max-width:240px;}
.km-graph-search{width:100%;padding:8px 32px 8px 12px;border-radius:10px;border:1px solid var(--border);background:var(--white);font-size:13px;color:var(--text);outline:none;transition:all 0.2s;}
.km-graph-search:focus{border-color:#2563eb;box-shadow:0 0 0 3px rgba(37,99,235,0.06);}
.km-graph-search-clear{position:absolute;right:8px;top:50%;transform:translateY(-50%);border:none;background:none;color:var(--text-dim);cursor:pointer;padding:4px;}
.km-graph-search-clear:hover{color:var(--text-md);}
.km-graph-actions{display:flex;align-items:center;gap:8px;}
.km-btn-icon{width:34px;height:34px;border-radius:8px;border:1px solid var(--border);background:var(--white);color:var(--text-dim);display:flex;align-items:center;justify-content:center;cursor:pointer;transition:all 0.2s;}
.km-btn-icon:hover{border-color:#2563eb;color:#2563eb;background:rgba(37,99,235,0.02);}
.km-btn-off{opacity:0.5;background:var(--light);}
.km-graph-filter{padding:7px 12px;border-radius:8px;border:1px solid var(--border);background:var(--white);color:var(--text-md);font-size:12px;font-weight:500;outline:none;cursor:pointer;max-width:140px;transition: var(--bg-trans);}
...
.knowledge-loading-overlay, .knowledge-error-overlay{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,0.1);backdrop-filter:blur(4px);z-index:20;}
.knowledge-loading-card, .knowledge-error-card{padding:24px 32px;border-radius:16px;background:var(--white);border:1px solid var(--border);box-shadow:0 10px 25px -5px rgba(0,0,0,0.05);display:flex;flex-direction:column;align-items:center;gap:12px;text-align:center;max-width:280px;transition: var(--bg-trans);}
.knowledge-loading-text{font-size:13px;font-weight:500;color:var(--text-md);}
...
.confidence-btn{width:32px;height:32px;border-radius:8px;border:1px solid var(--border);background:var(--white);cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all .15s;font-size:14px;font-weight:600;}
.confidence-btn:hover{background:var(--light);border-color:var(--orange);}

.confidence-modal-overlay{position:fixed;inset:0;background:rgba(24,24,46,0.45);backdrop-filter:blur(4px);z-index:1000;display:flex;align-items:center;justify-content:center;padding:20px;}
.confidence-modal{background:var(--white);border:1.5px solid var(--border-gold);border-radius:18px;width:100%;max-width:400px;box-shadow:0 20px 60px rgba(240,101,0,0.15),0 4px 20px rgba(0,0,0,0.1);display:flex;flex-direction:column;overflow:hidden;}
.confidence-modal-header{display:flex;align-items:flex-start;justify-content:space-between;padding:18px 20px 14px;border-bottom:1.5px solid var(--border-gold);}
.confidence-modal-title{font-family:var(--serif);font-size:19px;font-style:italic;background:var(--grad-text);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;}
.confidence-modal-close{width:30px;height:30px;border-radius:8px;border:1px solid var(--border-gold);background:var(--light);color:var(--text-dim);display:flex;align-items:center;justify-content:center;cursor:pointer;transition:all .15s;font-size:18px;font-weight:700;}
.confidence-modal-close:hover{border-color:var(--orange);color:var(--orange);}
.confidence-modal-content{padding:18px 20px 22px;display:flex;flex-direction:column;gap:12px;}
.confidence-score-display{font-size:48px;font-weight:700;text-align:center;background:var(--grad-text);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;margin:8px 0;}
.confidence-bar{height:10px;background:var(--light);border-radius:5px;overflow:hidden;}
.confidence-bar-fill{height:100%;border-radius:5px;transition:width .5s ease;}
.confidence-description{font-size:14px;color:var(--text-dim);text-align:center;line-height:1.6;}

.km-node-modal-overlay{position:fixed;inset:0;background:rgba(24,24,46,0.45);backdrop-filter:blur(4px);z-index:1000;display:flex;align-items:center;justify-content:center;padding:20px;}
.km-node-modal{background:var(--white);border:1.5px solid var(--border-gold);border-radius:18px;width:100%;max-width:850px;max-height:90vh;box-shadow:0 20px 60px rgba(240,101,0,0.15),0 4px 20px rgba(0,0,0,0.1);display:flex;flex-direction:column;overflow:hidden;position:relative;}
.km-node-modal-header{display:flex;flex-direction:column;gap:8px;padding:22px 24px 16px;border-bottom:1.5px solid var(--border-gold);}
.km-node-modal-title{font-family:var(--serif);font-size:26px;font-style:italic;background:var(--grad-text);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;}
.km-node-modal-close{position:absolute;top:20px;right:20px;width:36px;height:36px;border-radius:10px;border:1px solid var(--border-gold);background:var(--light);color:var(--text-dim);display:flex;align-items:center;justify-content:center;cursor:pointer;transition:all 0.15s;font-size:22px;font-weight:700;}
.km-node-modal-close:hover{border-color:var(--orange);color:var(--orange);}
.km-node-modal-content{padding:24px;overflow-y:auto;flex:1;}
.km-node-modal-source{font-size:14px;color:var(--text-dim);margin-bottom:12px;}
.km-node-modal-chunk-text{font-size:17px;color:var(--text);line-height:2.2;white-space:pre-wrap;word-wrap:break-word;}
